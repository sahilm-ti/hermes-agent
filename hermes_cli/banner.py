...
def check_for_updates() -> Optional[int]:
    ...
    # Read cache — invalidate if the embedded rev or installed version changed.
    # The stored baseline is restored with the cached result so user-facing
    # update guidance stays aligned with the count's target ref.
    now = time.time()
    try:
