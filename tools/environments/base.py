...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        #
        # The temp name MUST be unique per concurrent writer. ``$$`` is unsafe:
        # in ``&``-launched subshells it stays the parent shell's PID, so
        # concurrent writers can clobber the same temp file and publish a tear.
        # ``$BASHPID`` is unique on newer bash, but it's unset on macOS's
        # legacy bash 3.2. Keep ``$BASHPID`` in the token and append a per-write
        # random nonce so older shells still get unique temp names.
        #
        # Static path stays shell-quoted (Windows/Git-Bash paths, spaces) with
        # shell vars outside the quotes so they still expand.
        _snap_tmp_expr = (
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # Assemble into a unique temp file, then mv to atomically replace the
        # snapshot so concurrent source() calls never read a torn write.
        # ``$BASHPID`` is used when available; a random suffix keeps names unique
        # on bash 3.2 where ``$BASHPID`` is unset.
        _snap_tmp_expr = (
            self._quote_shell_path(self._snapshot_path + ".tmp.")
            + "$BASHPID.$RANDOM$RANDOM"
        )
